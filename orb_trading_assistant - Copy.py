...
from ai_reasoning_engine import LocalAIReasoningEngine, HybridReasoningEngine
...
class ORBTradingAssistant:
    ...
    def check_for_setup(self, data):
        ...
        pre_retest = post_or.iloc[-6:-1] if len(post_or) > 5 else post_or.iloc[:-1]
        latest['features'] = self.extractor._extract_features(pre_retest, latest)
        ...
        try:
            ...
            current_candle_copy = current_candle.copy()  # Make explicit copy
            
            # Extract features with NaN handling
            features = self.extractor._extract_features(pre_retest, current_candle_copy)
            ...
            current_candle_copy['features'] = features
        ...
        from ai_reasoning_engine import RuleBasedReasoningEngine
        rule_engine = RuleBasedReasoningEngine()
        self.reasoning_engine = rule_engine

        try:
            # Make sure pattern_model is available and properly initialized
            if self.pattern_model is not None and hasattr(self.pattern_model, 'analyze_pattern'):
                analysis = self.reasoning_engine.analyze_setup(current_candle_copy, current_market, self.pattern_model)
            else:
                # Create a simpler analysis without pattern model
                analysis = self.reasoning_engine.analyze_setup(current_candle_copy, current_market, None)
            
        except Exception as e:
            logger.error(f"Error in setup analysis: {str(e)}")
            # Try to run without pattern model if that was the issue
            try:
                logger.info("Retrying analysis without pattern model")
                analysis = self.reasoning_engine.analyze_setup(current_candle_copy, current_market, None)
            except Exception as e2:
                logger.error(f"Analysis failed again: {str(e2)}")
                self.reasoning_engine = temp_reasoning_engine  # Restore original engine
                continue

        # Always restore the original reasoning engine
        self.reasoning_engine = temp_reasoning_engine
    ...
    def backtest(self, symbol, start_date, end_date, api_key=None):
        ...
        if api_key is None and self.client is not None:
            api_key = self.client._session.params.get('apiKey')
        ...
        for day in trading_days:
            ...
            for i in range(5, len(day_data)):
                ...
                current_candle = current_market.iloc[-1]
                ...
                # Extract features
                pre_retest = current_market.iloc[-6:-1] if len(current_market) > 5 else current_market.iloc[:-1]
                current_candle['features'] = self.extractor._extract_features(pre_retest, current_candle)
    ...
    def optimize_parameters(self, symbol, start_date, end_date, api_key=None):
        """Optimize strategy parameters using backtest results"""
        logger.info(f"Optimizing parameters for {symbol} from {start_date} to {end_date}")
        
        # Parameter ranges to test
        profit_targets = [0.3, 0.5, 0.7, 1.0]
        stop_losses = [0.2, 0.25, 0.3, 0.4]
        ...
        for profit_target in profit_targets:
            for stop_loss in stop_losses:
                for volume_threshold in volume_thresholds:
                    logger.info(f"Testing parameters: PT={profit_target}%, SL={stop_loss}%, Vol={volume_threshold}x")
                    
                    # Create new extractor with these parameters
                    self.extractor = ORBPatternExtractor(
                    ...
                    # Run backtest
                    result = self.backtest(symbol, start_date, end_date, api_key)
                    
                    # Update best parameters if better
                    if result['profit_factor'] > best_profit_factor and result['win_rate'] > 0.4:
                        ...
                        logger.info(f"Profit factor: {best_profit_factor:.2f}, Win rate: {result['win_rate']*100:.2f}%")
        
        # Reset extractor to best parameters
        if best_params:
            self.extractor = ORBPatternExtractor(
            ...
            logger.warning("No profitable parameter combination found")
            return None